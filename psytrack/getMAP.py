...
from aux.auxFunctions import DT_X_D, sparse_logdet
...
def getMAP(dat, hyper, weights,
    ...
    K=0
    if type(weights) is not dict: raise Exception('weights must be a dict')
    ...
    if method is None:          w_N = N
    elif method == "_constant": w_N = 1   
    elif method == "_days":     w_N = len(dat['dayLength'])
    else: raise Exception("method type " + method + " not supported")


    # Initialize weights to particular values (default 0)       
    if E0 is not None:
        if type(E0) is not np.ndarray: raise Exception("E0 must be an array")
    ...
    if showOpt: print('Obtaining MAP estimate...')
    ...
    eMode = result.x
    Hess = lossfun.hessian(eMode,*my_args)  # dict of sparse components of Hess
    ...
    if showOpt: print('Calculating evd, first prior and likelihood at eMode...')
    pT, lT, wMode = getPosteriorTerms(eMode,*my_args)

    
    # Posterior term (with Laplace approx), calculating sparse log determinant
    if showOpt: print('Now the posterior with Laplace approx...')
    ...
    if showOpt: print("Evidence:", logEvd)
...
from aux.auxFunctions import read_input, make_invSigma, DTinv_v, myblk_diags
...
def getPosteriorTerms(E_flat, dat, hyper, weights, method=None):
    ...
    if 'dayLength' not in dat: dat['dayLength'] = np.array([], dtype=int)
    if 'missing_trials' not in dat: dat['missing_trials'] = None
