...
def getProb_1D(argStim, argParam):
	'''
	High level function that ultimately calculates the prob of each option y (usually pL & pR).
	argStim is usually a trail from x, as a 2D row vector
	argParam is a 1D array of the weights for this trial, w
	argDims is a dictionary with the dimensions of g (the input, usually 3) and y (the output, usually 1)
	'''
	pR = 1/(1 + np.exp(-argStim @ argParam))
...
def myblk_diags(A):
	'''
	Function to take (N,K,K) matrix A and put each len(N) entry of A[:,i,j] as the diagonal of
	an (N*K x N*K) matrix. For speed, sparsity is used — define the diagonal values of the final matrix
	then use diags to construct the final matrix. Some minor Python black magic is used below, 
	primarily the use of negative indices to index from the end, sorry.
	'''
	...
	# Need to keep track of each diagonal's offset in the final matrix : (0,1,...,K-1,-K+1,...,-1)
	...
	# After diagonals are constructed, use sparse function diags() to make matrix, then blow up to full size
...
def sparse_logdet(A):
	'''
	Calculate the log determinant using sparse LU decomposition, product of diagonal values of
	L and U matrix give determinant up to a sign change. Know positive determinant, so take
	log of the absolute value of the diagonal values and sum them for log-determinant.
	...
	aux = splu(A)
	return np.sum(np.log(np.abs(aux.L.diagonal()))) + np.sum(np.log(np.abs(aux.U.diagonal())))



def make_invSigma(hyper, days, missing_trials, N, K):
	# Setting a sigma value at index i, adjusts the change between trials i-1 and i
	...
	else: raise Exception('sigma must be of appropriate type, not' + str(type(sigma)))
...
def DT_X_D(ddlogprior, K):
	# Does the computation D.T @ ddlogprior @ D, where D is the blocked difference matrix
	# Much more quickly
